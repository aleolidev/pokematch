...
#     mon_names = [{"name": "Polla"}, {"name": "Bulba"}, {"name": "LOL"}]
#     print(type(mon_names))
...
#             get_stats(player_mon_name, player_struct)
...
#             get_stats(target_mon_name, target_struct)
...
#             # Get Icon
#             target_struct['icon'] = getattr(target_mon_name.sprites.versions, "generation-viii").icons.front_default
...
#     if re.match('^(\w+) (\w+) Face', player_mon_name) is not None:
#         player_mon_name = re.sub(r'(\w+) (\w+) (\w+)', r'\1 \2', player_mon_name)
#     if re.match('^(\w+) (\w+) Sword', player_mon_name) is not None:
#         player_mon_name = re.sub(r'(\w+) (\w+) (\w+)', r'\1 \2', player_mon_name)
#     if re.match('^(\w+) (\w+) Shield', player_mon_name) is not None:
#         player_mon_name = re.sub(r'(\w+) (\w+) (\w+)', r'\1 \2', player_mon_name)
#     if re.match('^(\w+) (\w+) (\w+) Style', player_mon_name) is not None:
#         player_mon_name = re.sub(r'(\w+) (\w+) (\w+) (\w+)', r'\1 \2 \3', player_mon_name)
#     player_mon_name = player_mon_name.lower().replace(' ', '-').replace('♀', '-f').replace('♂', '-m').replace("'", "")
#     return player_mon_name

# def get_stats(mon_name, mon_struct):
#     stats = mon_name.stats

#     # Normal stats
#     mon_struct['hp'] = stats[0].base_stat
#     mon_struct['atk'] = stats[1].base_stat
#     mon_struct['deff'] = stats[2].base_stat
#     mon_struct['sp_atk'] = stats[3].base_stat
#     mon_struct['sp_def'] = stats[4].base_stat
#     mon_struct['speed'] = stats[5].base_stat
...
#     # Min stats
#     mon_struct['min_hp'] = (stats[0].base_stat*2)+110
#     mon_struct['min_atk'] = floor(((stats[1].base_stat*2)+5)*0.9)
#     mon_struct['min_deff'] = floor(((stats[2].base_stat*2)+5)*0.9)
#     mon_struct['min_sp_atk'] = floor(((stats[3].base_stat*2)+5)*0.9)
#     mon_struct['min_sp_def'] = floor(((stats[4].base_stat*2)+5)*0.9)
#     mon_struct['min_speed'] = floor(((stats[5].base_stat*2)+5)*0.9)

#     # Max stats
#     mon_struct['max_hp'] = (stats[0].base_stat*2)+204
#     mon_struct['max_atk'] = floor(((stats[1].base_stat*2)+99)*1.1)
#     mon_struct['max_deff'] = floor(((stats[2].base_stat*2)+99)*1.1)
#     mon_struct['max_sp_atk'] = floor(((stats[3].base_stat*2)+99)*1.1)
#     mon_struct['max_sp_def'] = floor(((stats[4].base_stat*2)+99)*1.1)
#     mon_struct['max_speed'] = floor(((stats[5].base_stat*2)+99)*1.1)
